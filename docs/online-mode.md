...
Version 0.18.2 extends that protection across strike recovery. Once the locally predicted strike animation has completed, an older authoritative snapshot may still confirm its server-side recovery window but cannot replay the strike pose or reset the newly raised guard. Active, stunned, rejected, and interrupted strikes still reconcile to server state normally.
