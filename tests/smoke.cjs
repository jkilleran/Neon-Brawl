...
assert.equal(Object.keys(animationManifest.movements).length, 36, "Each fighter should expose 36 movements");
assert.equal(Object.keys(animationManifest.support).length, 9, "Support art should expose nine modular movements");
...
attacker.velocityX = -120;
assert.equal(attacker.getVisualFrame().animation, "footworkBackward", "Retreating should use its own fluid combat shuffle");
attacker.guard = "high";
attacker.guardVisual = "high";
attacker.guardBlend = 1;
attacker.velocityX = 120;
const highGuardAdvance = attacker.getVisualFrame();
assert.equal(highGuardAdvance.animation, "guardHighFootwork", "High guard should keep animated legs while advancing");
attacker.velocityX = -120;
const highGuardRetreat = attacker.getVisualFrame();
assert.equal(highGuardRetreat.animation, "guardHighFootwork", "High guard should keep animated legs while retreating");
assert.notEqual(highGuardRetreat.frame, highGuardAdvance.frame, "Guard retreat should reverse the reusable footwork cycle");
attacker.guard = "low";
attacker.guardVisual = "low";
assert.equal(attacker.getVisualFrame().animation, "guardLowFootwork", "Low guard should keep animated legs while moving");
attacker.velocityX = 0;
assert.equal(attacker.getVisualFrame().animation, "guardLow", "A stationary low guard should retain its defensive hold");
attacker.guard = null;
attacker.guardVisual = null;
attacker.guardBlend = 0;
...
assert.equal(imageSources.length, 76, "The layered arena and all 36 movements for both fighters should preload without the obsolete full plate");
